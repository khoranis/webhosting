        ...
        return this.each(function () {
            ...
            function addEventList() {
                $(Setting.PROVINCE).change(function (e) {
                    var PROVINCE_ID = $(this).val().split(" ")[0];
                    _loadAmphur(PROVINCE_ID);
                });
                $(Setting.AMPHUR).change(function (e) {
                    var AMPHUR_ID = $(this).val().split(" ")[0];
            ...
            function AddToView(list, key) {
                for (var i = 0; i < list.length; i++) {
                    if (key !== Setting.AMPHUR) {
                        $(key).append("<option value='" + list[i].id + " " + list[i].name + "'>" + list[i].name + "</option>");
                    } else {
                        $(key).append("<option value='" + list[i].id + " " + list[i].name + "' POSTCODE='" + list[i].postcode + "'>" + list[i].name + "</option>");
                    }
                }
            }
